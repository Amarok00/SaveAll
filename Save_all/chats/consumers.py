...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def save_message(self, message, username, chat_id):
        ...
        new_message = Message.objects.create(chat=chat, author=author, content=content)
    ...
    def get_avatar_url(self, username):
        try:
            ...
            if user.profile:
                # Assuming that the URL is stored in the 'image' field of the Profile model
                return user.profile.image.url or None  # or return a default avatar URL
            return None  # or handle the case when there is no profile
