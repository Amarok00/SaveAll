...
$(document).ready(function () {
    ...
    $(".comments-container").hide(); // hide comments by default
    $('.post-form input[type="file"]').hide(); // hide file upload input by default
    $('.post-form label[for="id_image"]').html(
        '<i class="fas fa-paperclip"></i>'
    ); // change label to paperclip icon
    $('.post-form label[for="id_image"]').attr(
        "title",
    ...
    // post and comment forms html for later use
    const commentFormHtml = `
        <input type="hidden" name="csrfmiddlewaretoken" value=${csrfToken}>
        <label for="id_content">Content:</label>
        <textarea name="content" cols="40" rows="3" maxlength="500" required="" id="id_content" spellcheck="false"></textarea>
    ...
    const editAvatarUrl =
        protocol + "//" + host + "/profiles/my_profile/edit_avatar/";
    ...
    const createPost = (e) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $(`.dislike-button[data-post-id='${post.id}']`).on(
                    "click",
                    dislikeHandler
                );
                $(`.comment-button[data-post-id='${post.id}']`).on(
                    "click",
                    commentHandler
                );
                $(`.comment-form[data-post-id='${post.id}']`).on(
                    "submit",
                    createComment
                );
                $(`.post[data-post-id='${post.id}'] img.post-image`).on(
                    "click",
                    toggleImage
                );
                $(`.edit-post-button[data-post-id='${post.id}']`).on(
                    "click",
                    toggleEditPost
                );
                $(`.delete-post-button[data-post-id='${post.id}']`).on(
                    "click",
                    toggleDeletePost
                );
                // enable all inputs
    ...
    const editPost = (e, postBackup) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $(`.dislike-button[data-post-id='${postId}']`).on(
                    "click",
                    dislikeHandler
                );
                $(`.comment-button[data-post-id='${postId}']`).on(
                    "click",
                    commentHandler
                );
                $(".edit-post-button[data-post-id=" + postId + "]").on(
                    "click",
                    toggleEditPost
                );
                $(".delete-post-button[data-post-id=" + postId + "]").on(
                    "click",
                    toggleDeletePost
                );
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                commentBackup
                    .find(".comment-time")
                    .find("em")
                    .text(`Edited at ${editedAt}`);
                $(e.target).replaceWith(commentBackup);
                $(".like-button[data-comment-id=" + commentId + "]").on(
                    "click",
                    likeHandler
                );
                $(".dislike-button[data-comment-id=" + commentId + "]").on(
                    "click",
                    dislikeHandler
                );
                $(".edit-comment-button[data-comment-id=" + commentId + "]").on(
                    "click",
                    toggleEditComment
                );
                $(".delete-comment-button[data-comment-id=" + commentId + "]").on(
                    "click",
                    toggleDeleteComment
                );
    ...
    const toggleEditPost = (e) => {
        ...
        let formElement = editPostFormHtml.replace(
            'data-post-id=""',
            'data-post-id="' + postId + '"'
        );
        ...
        if (postBackup[0] === lastPostOnThePage[0]) {
            $('.post-form.edit[data-post-id="' + postId + '"]').css(
                "margin-bottom",
                "7em"
            );
        }
        // insert the .post-text from the backup post into the edit post form textarea
        $('.post-form.edit[data-post-id="' + postId + '"]')
            .find("textarea")
            .val(postBackup.find(".post-text").text().trim());
        ...
        if (postBackup.find(".post-image").length > 0) {
            ...
            let labelBackup = $(
                '.post-form.edit[data-post-id="' + postId + '"]'
            ).find("label[for=post_file]");
            $('.post-form.edit[data-post-id="' + postId + '"]')
                .find("label[for=post_file]")
                .replaceWith(previewDiv);
        ...
        $(document).scrollTop(
            $('.post-form.edit[data-post-id="' + postId + '"]').offset().top -
            $(window).height() / 2
        );
        ...
        $('.post-form.edit[data-post-id="' + postId + '"]')
            ...
            .on("click", () => {
                $('.post-form.edit[data-post-id="' + postId + '"]').replaceWith(
                    postBackup
                );
                $('.edit-post-button[data-post-id="' + postId + '"]').on(
                    "click",
                ...
                $('.like-button[data-post-id="' + postId + '"]').on(
                    "click",
                    likeHandler
                );
                $('.dislike-button[data-post-id="' + postId + '"]').on(
    ...
    const toggleEditComment = (e) => {
        ...
        let formElement = editCommentFormHtml.replace(
            'data-comment-id=""',
        ...
        $('.comment-form.edit[data-comment-id="' + commentId + '"]')
            .find("textarea")
            .val(commentBackup.find(".comment-content").text().trim());
        ...
        $(document).scrollTop(
            $('.comment-form.edit[data-comment-id="' + commentId + '"]').offset()
            .top -
            $(window).height() / 2
        );
        // make form z-index higher than the cover div
        $('.comment-form.edit[data-comment-id="' + commentId + '"]').css(
            "z-index",
            "11"
        );
        // prevent the page from scrolling
        ...
        // need to add event handler to form submit
        $('.comment-form.edit[data-comment-id="' + commentId + '"]').on(
            "submit",
            (e) => {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        $(".post").each((index, element) => {
                            $(element)
                                .find('.post-avatar[src="' + currentAvatarUrl + '"]')
    ...
    const joinCommunityUrl =
        protocol + "//" + host + window.location.pathname + "join/";
    const leaveCommunityUrl =
        protocol + "//" + host + window.location.pathname + "leave/";
    ...
    const joinCommunity = (e) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                let leaveCommunityButton = $(
                    '<button class="leave-community-button" data-community-id="' +
                    communityId +
                    '">Leave community <i class="fas fa-sign-out-alt"></button>'
                );
    ...
    const leaveCommunity = (e) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                let joinCommunityButton = $(
                    '<button class="join-community-button" data-community-id="' +
                    communityId +
                    '">Join community <i class="fas fa-sign-in-alt"></button>'
                );
